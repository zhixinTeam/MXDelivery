...
uses
  ...
  cxDropDownEdit, cxLabel, ULEDFont, cxCheckBox;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((FType = sFlag_Dai) and (
         ...
         ((FType = sFlag_San) and ((Length(FSanVerifyStock)<1) and
          (nVal < 0) and (FPoundSanF > 0) and (-nVal > FPoundSanF))) or
         ((FType = sFlag_San) and ((Pos(FStockNo, FSanVerifyStock)>0) and
          (FPoundSanF > 0) and (Abs(nVal) > FPoundSanF))) then
      begin
        nStr := '车辆[%s]实际装车量误差较大，请通知司机检验载重';
        nStr := Format(nStr, [FTruck]);
        PlayVoice(nStr);

        nError := '车辆[ %s ]实际装车量误差较大,详情如下:' + #13#10#13#10 +
                  '※.开单量: %.2f吨' + #13#10 +
                  '※.装车量: %.2f吨' + #13#10 +
                  '※.误差量: %.2f公斤' +#13#10#13#10 +
                  '请通知司机点验载重.';
        nError := Format(nError, [FTruck, FInnerData.FValue, nNet, nVal]);
  ...
  begin
    ...
    if FHasErr then
         Result := SavePostError(FNextStatus, FBillItems, FPoundTunnel, nError)
    else Result := SaveLadingBills(FNextStatus, FBillItems, FPoundTunnel);
    //保存称重
  end;
end;
...
var nNextStatus, nError, nStr: string;
    nNet, nVal: Double;
begin
  Result := False;
  nError := '';
  ...
  if (FUIData.FPData.FValue > 0) and (FUIData.FMData.FValue <= 0) then
  begin
    nNet := GetTruckFullValue(FUIData.FTruck);
    nVal := nNet * 1000 - FUIData.FPData.FValue * 1000;

    if (nNet > 0) and (gSysParam.FPoundProM > 0) and
       (nVal < 0) and (-nVal > gSysParam.FPoundProM) then
    begin
      nStr := '车辆[%s]实时毛重误差较大,请下车通知门岗检验车厢';
      nStr := Format(nStr, [FUIData.FTruck]);
      PlayVoice(nStr);

      nError := '车辆[ %s ]实时毛重误差较大,详情如下:' + #13#10#13#10 +
                '※.实时毛重: %.2f吨' + #13#10 +
                '※.历史毛重: %.2f吨' + #13#10 +
                '※.误差量: %.2f公斤' + #13#10#13#10 +
                '是否继续保存?';
  ...
  if FHasErr then
       Result := SavePostError(nNextStatus, FBillItems, FPoundTunnel, nError)
  else Result := SaveLadingBills(nNextStatus, FBillItems, FPoundTunnel);
  //保存称重
end;
...
begin
  try
    ...
    FLastCard := FLastCardTmp;
    FLastCardDone := GetTickCount;
    if not FHasErr then PlayVoice(#9 + FUIData.FTruck);
    //播放语音
