...
begin     
  ...
  begin
    ...
    if FBillID = '' then
      FBillID := FieldByName('P_Order').AsString;
    //采购单号
  ...
  if FPoundType = sFlag_Provide then //供应
  begin
    nStr := 'Select P_Status,P_NextStatus From %s Where P_ID=''%s''';
    nStr := Format(nStr, [sTable_PurchInfo, FBillID]);

    with FDM.QueryTemp(nStr) do
    begin
      if (RecordCount < 1) or
         (FieldByName('P_Status').AsString = sFlag_TruckOut) then
      begin
        SetCtrlData(EditStatus, sFlag_TruckOut);
        EditMemo.Text := '采购业务已出厂,无法勘误';
        Exit;
      end;
...
begin
  ...
  nStr := MakeSQLByStr([SF('P_Truck', EditTruck.Text),
          ...
          SF('P_MValue', nMV, sfVal),
          SF('P_MDate', nMD, sfVal),
          SF('P_Status', GetCtrlData(EditStatus)),
          SF('P_NextStatus', GetCtrlData(EditNext))
          ], sTable_PurchInfo, SF('P_ID', FBillID), False);
  FDM.ExecuteSQL(nStr);
