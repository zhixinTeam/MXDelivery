...
begin
  ...
  if Check1.Checked then
  begin
    nStr := 'Select D_ID from %s where D_Stock=''%s'' and D_Valid=''%s'' And ' +
            'D_ID <> ''%s''';
    nStr := Format(nStr , [sTable_BatcodeDoc,
            GetCtrlData(EditStock), sFlag_BatchInUse, Trim(EditBatch.Text)]);
    //xxxxx

    with FDM.QuerySQL(nStr) do
    if RecordCount > 0 then
    begin
      nStr := '批次号[%s]正在使用,欲使用新批次,请先停用该批次';
      nStr := Format(nStr , [Trim(Fields[0].ASString)]);
